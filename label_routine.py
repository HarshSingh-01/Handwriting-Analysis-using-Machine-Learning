...
def determine_trait_6(letter_size, line_spacing):
    # trait_6 = poor concentration power | 1 = observed, 0 = not onbserved (not necessarily the opposite)
...
feature_list_path = "C:\\Users\Harsh\Desktop\Projects\Handwriting-Analysis-using-Machine-Learning\Extracted_Features/feature_list.txt"

if os.path.isfile(label_list):
        print ("Error: label_list already exists.")

elif os.path.isfile(feature_list_path):
    print ("Info: feature_list found.")
    with open(feature_list_path, "r") as features, open(label_list, "a") as labels:
        for line in features:
            ...
            labels.write("%s\t%s\t%s\t%s\t%s\t%s\t" % (str(baseline_angle), str(top_margin), str(letter_size), str(line_spacing), str(word_spacing), str(slant_angle)))
            labels.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t" % (str(trait_1), str(trait_3), str(trait_4), str(trait_5), str(trait_6), str(trait_7), str(trait_8)))
